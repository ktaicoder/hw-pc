...
import PortsView from 'src/render/components/PortsView'
...
export default function CodingpackInternalView(props: Props) {
  ...
  const openCodingpackActionDialog = (kind: CodingpackActionKindKey) => {
    if (!hwReady) {
  ...
  const handleClickDriverDownload = (driverPath: string) => {
  ...
  return (
    ...
    >
      <Box sx={{ margin: '0 auto', position: 'relative', width: '100%', maxWidth: 960 }} pt={10}>
            ...
            >
              {pcDrivers.map((driver, idx) => (
                <Button
                  ...
                  onClick={() => handleClickDriverDownload(driver.uri)}
                >
                  {driver.name}
                </Button>
              ))}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Button
                ...
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  OpenDocDialogEvent.send({ docId: 'hw-pc-how-to-codingpack-setup' })
                }}
              >
                설명서
              </Button>
              <Button
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  OpenDocDialogEvent.send({ docId: 'hw-pc-codingpack-setup-help' })
                }}
              >
                문제가 생겼나요?
              </Button>
            ...
            <Stack spacing={2}>
              {CodingpackActions.filter((it) => !DISABLED_KIND.includes(it.kind)).map(
                ({ title, subtitle, docId, kind }) => (
                  <CodingpackActionRow
                    key={kind}
                    onClick={() => openCodingpackActionDialog(kind)}
                    disabled={!hwReady}
                    docId={docId}
                    kind={kind}
                    title={title}

...
import { clamp, createNumberArray, sleepAsync } from '../util'
...
const output = new AltinoLightOutput()
...
export class SaeonAltinoLiteControl extends AbstractHwConrtol implements ISaeonAltinoLiteControl {
  ...
  async go(ctx: any, lp: number, rp: number): Promise<void> {
    ...
    output.LM = clamp(lp, -1000, 1000)
    output.RM = clamp(rp, -1000, 1000)
  }
  ...
  async steering(ctx: any, option: string): Promise<void> {
    ...
    output.STR = clamp(value, -127, 127)
  }
  ...
  async sound(ctx: any, oct: string, scale: string): Promise<void> {
    ...
    output.NOTE = nScale == 0 ? 0 : (nOct - 1) * 12 + nScale
    console.log('sound')
  }
  ...
  async displayChar(ctx: any, ch: string): Promise<void> {
    ...
    output.CHAR = character.charCodeAt(0)
  ...
  async displayLine(
    ...
    const maskFn = (bit: string, dmKey: string) => {
      const dm = output[dmKey]
  ...
  private forceStopAll_ = async (ctx: any) => {
    const logTag = 'SaeonAltinoLiteControl.forceStopAll_()'
  ...
  private writeOutput_ = async (ctx: any, logTag: string): Promise<any> => {
    ...
    this.txBytes[6] = (output.RM & 0xff00) >> 8
    this.txBytes[7] = output.RM & 0xff
    this.txBytes[8] = (output.LM & 0xff00) >> 8
    this.txBytes[9] = output.LM & 0xff
    this.txBytes[10] = output.CHAR
    this.txBytes[11] = output.DM1
    this.txBytes[12] = output.DM2
    this.txBytes[13] = output.DM3
    this.txBytes[14] = output.DM4
    this.txBytes[15] = output.DM5
    this.txBytes[16] = output.DM6
    this.txBytes[17] = output.DM7
    this.txBytes[18] = output.DM8
    this.txBytes[19] = output.NOTE
    this.txBytes[20] = output.LED

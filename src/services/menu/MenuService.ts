...
import type { IPreferencesService } from 'src/services/preferences/IPreferencesService'
import serviceIdentifier from 'src/services/serviceIdentifier'
import type { IWindowService } from 'src/services/windows/IWindowService'
...
export class MenuService implements IMenuService {
  @lazyInject(serviceIdentifier.Preferences)
  private readonly preferenceService!: IPreferencesService
  ...
  private belongsToPart(menuPartKey: string, menuID: string, itemID?: string): boolean {
    ...
    if (record !== undefined) {
      return record.some(
        ([currentMenuID, currentItemID]) => menuID === currentMenuID && itemID === currentItemID,
      )
    }
    return false
  }
  ...
  public async buildMenu() {
    const latestTemplate =
      (await this.getCurrentMenuItemConstructorOptions(this.menuTemplate)) ?? []
    try {
      const menu = Menu.buildFromTemplate(latestTemplate)
      Menu.setApplicationMenu(menu)
    } catch (error) {
      logger.error(
        `buildMenu() failed: ${(error as Error).message} ${
  ...
  private static isMenuItemEqual<
    T extends DeferredMenuItemConstructorOptions | MenuItemConstructorOptions,
  >(a: T, b: T): boolean {
  ...
  public async insertMenu(
    ...
    for (const menu of this.menuTemplate) {
      ...
      if (menu.id === menuID) {
        ...
        let filteredSubMenu: Array<
          DeferredMenuItemConstructorOptions | MenuItemConstructorOptions
        > = currentSubMenu
